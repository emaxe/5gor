...
export class SkidMarks {
  ...
  update(player, world) {
    ...
    // Высота поверхности: на Машуке (серпантин) heightAt даёт реальный рельеф;
    // в городе (heightAt = 0) асфальт/тротуар — плоские короба с верхом 0.10/0.15,
    // поэтому след поднимаем над ними, иначе он тонет под асфальтом и не виден.
    const hy = world.heightAt(player.x, player.z);
    const y = hy > 0.5 ? hy + 0.03 : 0.16; // зазор против z-fighting (верх тротуара 0.15)
