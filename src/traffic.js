...
export class TrafficManager {
  ...
  _chooseDirection(car, isec) {
    ...
    // мировая точка старта и финиша. Финиш смещаем ВПЕРЁД по новой дороге на
    // дистанцию выезда (4.5 м), иначе для правого поворота to оказывается в центре
    // перекрёстка, хорда мала, кривая вырождается и машина «поворачивает через
    // три левых» (нос уводит вбок, а в конце резко дёргает в нужный курс).
    const from = this._worldPos(car, _tempTrafficFrom);
    const exitPos = (car.axis === 'z' ? isec.x : isec.z) + newDir * 4.5;
    const to = this._worldPos({ axis: newAxis, coord: newCoord, pos: exitPos, dir: newDir }, _tempTrafficTo);
    ...
    car.turn = {
      ...
      newAxis, newCoord, newPos: exitPos, newDir,
    };
