  ...
  update(dt, player, world, density, peds) {
    ...
    for (const car of this.cars) {
      ...
      if (car.turn) {
        ...
        // Направление по касательной, развёрнутое непрерывно от стартового курса.
        // atan2 возвращает [-π,π], а курс использует π для -Z; смешивать их через
        // lerpAngle нельзя — нормализация кратчайшей дуги выбирает длинный путь и
        // машина «крутится как юла». Поэтому берём касательную и разворачиваем её
        // в диапазон [fromH-π, fromH+π], чтобы нос всегда смотрел по ходу движения.
        let h = len > 1e-5 ? Math.atan2(dx, dz) : T.fromH;
        let d = h - T.fromH;
        while (d > Math.PI) d -= Math.PI * 2;
        while (d < -Math.PI) d += Math.PI * 2;
        car.mesh.rotation.y = T.fromH + d;
