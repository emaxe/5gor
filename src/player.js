...
export class PlayerCar {
  ...
  _applyTuning(tuning) {
    ...
    // стиль диска: геометрия всех 3 вариантов уже построена в _build()
    // (wheels[i].rimVariants) — переключаем только видимость, без пересборки
    const rimStyle = tuning.rimStyle || 'disc';
    for (const w of this.wheels) {
      if (!w.rimVariants) continue;
  ...
  _collide(world, traffic) {
    // Капсула-коллайдер машины: вместо одного круга r=2.0 (который на 1 м торчит
    // за борта) используем ТРИ круга вдоль оси машины — радиус = полуширина
    // кузова, позиции перед/центр/зад. Это повторяет прямоугольный силуэт:
    //   taxi (1.9×4.3):  rc=0.97, sep=1.18 → перед/зад 2.15м, борт 0.97м
    // Боковая досягаемость = ровно ширина кузова в любой точке оси, поэтому
    // "врезаться в столб/сбить животное" можно только реально касаясь кузова,
    // а не проезжая в метре от него. Три круга (а не два) обязательны: при
    // двух кругах с радиусом=полуширина у длинной машины (L>2W) в центре был бы
    // провал, через который пешеход мог бы пройти насквозь.
    const shape = CAR_TYPE_SHAPE[this.stats.carType] || CAR_TYPE_SHAPE.taxi;
    const halfW = shape.w / 2;
    const halfL = shape.len / 2;
    const rc = halfW * 1.03;
    const sep = halfL - halfW;            // смещение переднего/заднего круга
    const fwdX = Math.sin(this.heading), fwdZ = Math.cos(this.heading);

    // Три центра капсулы (перед/центр/зад) в мировых координатах
    const offs = [sep, 0, -sep];
    for (let i = 0; i < offs.length; i++) {
      const off = offs[i];
      this._resolveAt(this.x + fwdX * off, this.z + fwdZ * off, rc, sep, world, traffic);
    }
  }
  ...
  _resolveAt(px, pz, rc, sep, world, traffic) {
    ...
    for (const cld of world.circleColliders) {
      const d = dist2D(px, pz, cld.x, cld.z);
      if (d < cld.r + rc) {
        if (d > 1e-6) {
          _tempPlayerCircleRes.nx = (px - cld.x) / d;
          _tempPlayerCircleRes.nz = (pz - cld.z) / d;
        } else {
          _tempPlayerCircleRes.nx = 0;
          _tempPlayerCircleRes.nz = 1;
        }
        _tempPlayerCircleRes.depth = cld.r + rc - d;
    ...
    if (traffic) {
      for (const car of traffic.cars) {
        ...
        const d = dist2D(px, pz, car.x, car.z);
        const rr = rc + car.radius;
        if (d < rr && d > 1e-4) {
          const nx = (px - car.x) / d, nz = (pz - car.z) / d;
          // расталкивание: капсульный круг упирается в трафик-авто — двигаем
          // ВЕСЬ центр машины на тот же вектор, что и круг (круг смещён на
          // sep от центра, поэтому центр машины сдвигаем на (rr-d))
          const delta = rr - d;
          this.x += nx * delta; this.z += nz * delta;
  ...
  _collidePed(p, rc, px, pz, world) {
    if (p.alive === false || p.x === undefined || p.z === undefined) return;
    if (p.mesh && !p.mesh.visible) return;
    const d = dist2D(px, pz, p.x, p.z);
    const rr = rc + 0.45;                     // запас на "толщину" пешехода
    if (d >= rr || d < 1e-4) return;
    const nx = (p.x - px) / d, nz = (p.z - pz) / d;
    // выталкиваем из-под машины на границу контакта (в мировых координатах)
    const wx = px + nx * rr, wz = pz + nz * rr;
    p.x = wx;
    p.z = wz;
    if (p.mesh) {
