...
export class UIManager {
  ...
  updateHud(player, gameState, orders, hour, camera, world, gpsRoute, fuelRoute, gpsTargetType) {
    ...
    if (a) {
      ...
      // Ночью фактическая выплата умножается на nightMult (orders.js complete),
      // но estPay считается при спавне без множителя. Показываем честную оценку
      // с учётом ночного тарифа, чтобы игрок не видел вдвое заниженную цифру.
      const night = hour >= CFG.nightStartHour || hour < CFG.nightEndHour;
      const shownPay = night ? Math.round(a.estPay * CFG.nightMult) : a.estPay;
      els['order-pay'].textContent = '≈ ' + fmtMoney(shownPay) + (night ? ' (ночь ×' + CFG.nightMult + ')' : '') + ' + чаевые';
    } else {
