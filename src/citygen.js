...
export class World {
  ...
  _roads() {
    ...
    // Разметка ТОЛЬКО в пределах дорожного полотна (±256 м): штрихи за городской
    // границей падали на траву (баг «разметка выходит на траву»).
    const ROAD_EDGE = 256;
    const dashesV = [];
    for (const r of this.roadsV) {
      for (let z = -ROAD_EDGE; z <= ROAD_EDGE; z += 6.4) {
    ...
    const dashesH = [];
    for (const r of this.roadsH) {
      for (let x = -ROAD_EDGE; x <= ROAD_EDGE; x += 6.4) {
  ...
  _crosswalks() {
    ...
    for (const isec of this.intersections) {
      ...
      // Городские границы: дороги идут только до ±256 м, за ними трава. У граничных
      // перекрёстков зебра рисуется ТОЛЬКО на внутреннем подходе — внешняя полоса
      // (|coord| + 6.2) падала на траву за пределами дорожного полотна.
      const atNorth = sz <= -256, atSouth = sz >= 256; // верх/низ карты
      const atWest = sx <= -256, atEast = sx >= 256;      // лево/право карты
      // 4 зебры вокруг каждого перекрёстка
      for (let k = -2; k <= 2; k++) {
        const off = k * 1.1;
        // Верхний и нижний подходы к перекрёстку (полосы вдоль X)
        if (!atNorth) listV.push(new THREE.Matrix4().makeTranslation(sx + off, 0.11, sz - 6.2));
        if (!atSouth) listV.push(new THREE.Matrix4().makeTranslation(sx + off, 0.11, sz + 6.2));

        // Левый и правый подходы к перекрёстку (полосы вдоль Z)
        if (!atWest) listH.push(new THREE.Matrix4().makeTranslation(sx - 6.2, 0.11, sz + off));
        if (!atEast) listH.push(new THREE.Matrix4().makeTranslation(sx + 6.2, 0.11, sz + off));
      }
    }
