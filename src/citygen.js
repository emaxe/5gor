...
import { mulberry32, dist2D, rand, clamp, choice, makeCanvas, canvasToTexture, lerp, mergeColored, makePlateTexture, makeTaxiTexture, getWindowMaterial, getRoofMaterial, smin, smax } from './utils.js';
...
export class World {
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dz = -1; dz <= 1; dz++) {
        ...
        for (const p of bucket) {
          if (x > p.x0 - radius && x < p.x1 + radius && z > p.z0 - radius && z < p.z1 + radius) return true;
        }
      }
    }
    return false;
  ...
  _gazebo(x, z, y) {
    ...
    this.scene.add(g);
    this.addPropAABB({ x0: x - 2.4, z0: z - 2.4, x1: x + 2.4, z1: z + 2.4 });
  }
  ...
  _tramStop(x, z) {
    ...
    g.position.set(x, 0, z);
    this.scene.add(g);
    this.addPropAABB({ x0: x - 2.2, z0: z - 1.2, x1: x + 2.2, z1: z + 1.2 });
  }
  ...
  _parkCvetnik() {
    ...
    this.addPropAABB({ x0: cx - 6.6, z0: cz - 6.6, x1: cx + 6.6, z1: cz + 6.6 });
    // клумбы
    ...
    for (let i = 0; i < 6; i++) {
      ...
      this.scene.add(b);
      this.addPropAABB({ x0: b.position.x - 2.5, z0: b.position.z - 0.8, x1: b.position.x + 2.5, z1: b.position.z + 0.8 });
    }
  ...
  _market() {
    ...
    for (let ri = 0; ri < 3; ri++) for (let ci = 0; ci < 4; ci++) {
      ...
      this.addPropAABB({ x0: x - 4.4, z0: z - 2.6, x1: x + 4.4, z1: z + 2.6 });
    }
    for (let i = 0; i < 8; i++) {
      const cx = 79 + this.rng() * 34, cz = -10 + this.rng() * 8;
      crateM.push(new THREE.Matrix4().makeTranslation(cx, 0.5, cz));
      this.addPropAABB({ x0: cx - 0.6, z0: cz - 0.6, x1: cx + 0.6, z1: cz + 0.6 });
    }
  ...
  _station() {
    ...
    this.scene.add(tm);
    this.addPropAABB({ x0: 137, z0: 102.5, x1: 159, z1: 109.5 });
  }
  ...
  _trees() {
    ...
    const tryAddTree = (x, z, type) => {
      ...
      spots.push({ x, z, type });
      this.addPropAABB({ x0: x - 1.6, z0: z - 1.6, x1: x + 1.6, z1: z + 1.6 });
    };
    ...
    for (const r of this.roadsV) {
      ...
      for (let z = Z0; z <= 224 + 24; z += step) {
        ...
        if (this.isPositionValid(lx, z, 0.5)) {
          ...
          this.addPropAABB({ x0: lx - 0.4, z0: z - 0.4, x1: lx + 0.4, z1: z + 0.4 });
        }
        side = -side;
      }
    }
    for (const r of this.roadsH) {
      ...
      for (let x = Z0; x <= 224 + 24; x += step) {
        ...
        if (this.isPositionValid(x, lz, 0.5)) {
          ...
          this.addPropAABB({ x0: x - 0.4, z0: lz - 0.4, x1: x + 0.4, z1: lz + 0.4 });
        }
        side = -side;
      }
    }
  ...
  _fuelStations() {
    ...
    for (const s of this.fuelStations) {
      ...
      this.addPropAABB({ x0: x - 4.2, z0: z - 0.6, x1: x + 4.2, z1: z + 0.6 });
      this.addPropAABB({ x0: x - 0.8, z0: z + 1.2, x1: x + 0.8, z1: z + 2.4 });
  ...
  _cableCar() {
    ...
    // опоры — точки подобраны в стороне от коридора серпантина (dist > 15м)
    for (const t of [0.28, 0.6, 0.85]) {
      const p = new THREE.Vector3().lerpVectors(base, top, t);
      const h = Math.max(4, p.y - this.heightAt(p.x, p.z) + 4);
    ...
    st.position.set(20, this.heightAt(20, -288) + 2.6, -288);
    this.scene.add(st);
    this.addPropAABB({ x0: 14, z0: -292, x1: 26, z1: -284 });
  }
