...
export class AudioManager {
  ...
  _bindEvents() {
    ...
    // Наградные события: фанфара достижения, зинг заноса, звон идеальной остановки
    Events.on('achievement:unlocked', () => {
      if (!this._ready) return;
      this.sfx.achievementFanfare();
    });
    Events.on('drift:completed', (d) => {
      if (!this._ready) return;
      this.sfx.comboMilestone(d && d.duration ? Math.min(3, Math.floor(d.duration / 2)) : 1);
    });
