...
export const CFG = {
  ...
  // Уровень розыска (wanted level): повторные нарушения перед патрулём
  // повышают уровень 1-5, увеличивая штрафы и радиус детекции полиции.
  WANTED: {
    maxLevel: 5,
    decayTime: 25,          // сек на спад одного уровня без новых нарушений
    fineMultPerLevel: 0.5,  // L2 => x1.5, L5 => x3.0 исходного штрафа
    ratingMultPerLevel: 0.5,
    detectRadiusBonus: 15,  // +м к радиусу детекции за каждый уровень выше 1
    chaseLevel: 4,          // с этого уровня ближайший патруль переходит в преследование
    escapeRewardPerLevel: 150, // ₽ награды за успешный уход от полиции за каждый уровень розыска
    escapeRatingBonus: 2,   // +рейтинг за успешный уход от полиции за каждый уровень
  },
  ...
  // Опасное сближение (near-miss): награда за проезд вплотную к NPC на скорости
  // без касания. Зеркалит дрифт/perfect-stop — аддитивно, физику не трогает.
  nearMissMinSpeed: 10.0,   // мин. скорость игрока (м/с, ~36 км/ч) — не награждать в пробке
  nearMissCarMargin: 1.2,   // макс. боковой зазор между кузовами игрока и авто (м)
  nearMissPedMargin: 1.0,   // макс. зазор между кузовом игрока и пешеходом (м)
  nearMissResetDist: 8.0,   // дистанция удаления для сброса одноразового флага NPC (м)
  nearMissReward: 35,       // базовая выплата за опасное сближение (₽)
  nearMissStyleBonus: 0.04, // лёгкая компенсация штрафа стиля пассажира (+4% к style)
  nearMissStreakWindow: 8.0, // окно времени между сближениями для удержания серии (сек)
  nearMissStreakTiers: [     // пороги серии сближений с множителем награды (по возрастанию)
    { count: 2,  mult: 2,  level: 1 },
    { count: 5,  mult: 5,  level: 2 },
